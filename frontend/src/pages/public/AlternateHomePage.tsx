...
const proofSteps = [
  { id: "evidence", label: "Evidence", title: "What people know", body: "Different people and systems hold different parts of the answer.", image: "/images/homepage-proof-evidence-placeholder.svg", alt: "Placeholder for accepted Golden Proof Thread evidence capture" },
  { id: "picture", label: "Picture", title: "What the picture shows", body: "StrategicAI connects the people, work, systems, dependencies, and unknowns around the same problem.", image: "/images/homepage-proof-picture-placeholder.svg", alt: "Placeholder for accepted Golden Proof Thread operating picture capture" },
  { id: "correction", label: "Correction", title: "What gets corrected", body: "The owner can say: “No. That’s not how it works.” And the picture changes.", image: "/images/homepage-proof-correction-placeholder.svg", alt: "Placeholder for accepted Golden Proof Thread owner correction capture" },
  { id: "question", label: "Question", title: "What you can ask next", body: "Now a real question can follow the relationships instead of starting from a blank prompt.", image: "/images/homepage-proof-question-placeholder.svg", alt: "Placeholder for accepted Golden Proof Thread context-aware Nemo capture" },
] as const;
...
const businessViews = [
  ...
  { id: "handoffs", label: "Who needs what from whom?", detail: "Follow the relationships where information, authority, or timing has to move between people." },
...
const nemoQuestions = [
  { id: "owner-dependence", question: "What still depends on me?", response: "Trace the work, authority, and handoffs that keep making you the default point of return." },
...
function ProofFrame({ step, onOpenImage }: { step: ProofStep; onOpenImage: (src: string) => void }) {
  return (
    <button type="button" onClick={() => onOpenImage(step.image)} className="group block w-full overflow-hidden rounded-[2rem] border border-slate-800 bg-slate-950/70 text-left transition hover:border-cyan-400/40" aria-label={`Open larger ${step.label.toLowerCase()} proof frame`}>
      <div className="relative aspect-[16/10] overflow-hidden">
        <img src={step.image} alt={step.alt} className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.02]" />
        <span className="absolute bottom-4 right-4 inline-flex items-center gap-2 rounded-full border border-white/20 bg-slate-950/80 px-3 py-2 text-xs font-semibold text-white backdrop-blur"><Eye size={14} aria-hidden="true" /> View frame</span>
      </div>
    </button>
  );
}
...
export default function AlternateHomePage() {
  ...
  function toggleProof() {
    ...
    if (activeProofStep >= proofSteps.length - 1) setActiveProofStep(0);
    setProofPlaying(true);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white selection:bg-cyan-400/20">
      <main>
        ...
        <section className={`${sectionClass} pt-0`} aria-labelledby="reframe-title"><div className="grid items-stretch gap-8 lg:grid-cols-[1.05fr_0.95fr]"><div className={cardClass}><p className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] text-cyan-300/75">The Reframe</p><h2 id="reframe-title" className="text-3xl font-semibold text-white md:text-5xl">Your business already has an operating model. It just may not be visible.</h2><p className="mt-6 text-lg leading-8 text-slate-300">The real business lives in the relationships between people, work, systems, decisions, exceptions, and history. Before you add another tool—or automate another step—you need to see that picture clearly enough to challenge it.</p></div><div className="flex flex-col justify-center rounded-3xl border border-amber-200/25 bg-[linear-gradient(145deg,rgba(70,55,30,0.4),rgba(15,23,42,0.86))] p-8 md:p-10"><p className="text-sm font-semibold uppercase tracking-[0.24em] text-amber-200/80">Automation Without Understanding</p><p className="mt-6 text-3xl font-semibold leading-tight text-white md:text-4xl">Start with the picture. Then decide what deserves action.</p><div className="mt-8 flex flex-wrap items-center gap-3 text-sm text-slate-300">{["clarify ownership", "fix a handoff", "investigate", "automate when warranted", "leave it alone"].map((item) => <span key={item} className="rounded-full border border-slate-700 bg-slate-950/30 px-3 py-2">{item}</span>)}</div></div></div></section>

        <section id="proof-thread" className={`${sectionClass} scroll-mt-20`} aria-labelledby="proof-thread-title"><div className="mb-10 max-w-3xl"><p className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] text-cyan-300/75">See the Move</p><h2 id="proof-thread-title" className="text-3xl font-semibold text-white md:text-5xl">Watch one problem become something you can actually inspect.</h2><p className="mt-6 text-lg leading-8 text-slate-400">Follow the same business, the same problem, and the same story from what people know → the picture StrategicAI forms → the owner’s correction → the next question Nemo can follow.</p></div><div className="grid gap-8 lg:grid-cols-[0.82fr_1.18fr]"><div className="space-y-4"><div className="flex items-center justify-between border-b border-slate-800 pb-5 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500"><span>Golden Proof Thread</span><span>45 seconds</span></div><div className="flex flex-wrap gap-2 text-xs text-slate-400"><span className="rounded-full border border-slate-800 bg-slate-900/50 px-3 py-2">one business</span><span className="rounded-full border border-slate-800 bg-slate-900/50 px-3 py-2">one real problem</span><span className="rounded-full border border-slate-800 bg-slate-900/50 px-3 py-2">same story all the way through</span></div>{proofSteps.map((step, index) => { const active = index === activeProofStep; return <button type="button" key={step.id} onClick={() => selectProofStep(index)} aria-pressed={active} className={`w-full rounded-2xl border p-5 text-left transition ${active ? "border-cyan-400/40 bg-cyan-400/10" : "border-slate-800 bg-slate-900/35 hover:border-slate-700"}`}><div className="flex items-center gap-4"><span className="text-sm font-semibold uppercase tracking-[0.2em] text-cyan-300/80">0{index + 1}</span><span className="text-lg font-semibold text-white">{step.label}</span></div><p className="mt-3 text-sm leading-6 text-slate-400">{step.title}</p></button>; })}<div className="flex flex-col gap-3 pt-2 sm:flex-row"><button type="button" onClick={startProof} className="inline-flex items-center justify-center rounded-full bg-cyan-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300"><Play size={15} className="mr-2" aria-hidden="true" /> Play the 45-second proof</button><button type="button" onClick={toggleProof} className="inline-flex items-center justify-center rounded-full border border-slate-700 px-5 py-3 text-sm font-semibold text-white transition hover:border-slate-500 hover:bg-slate-900">{proofPlaying ? <Pause size={15} className="mr-2" aria-hidden="true" /> : <Play size={15} className="mr-2" aria-hidden="true" />}{proofPlaying ? "Pause proof" : "Start at this step"}</button></div></div><div aria-live="polite"><ProofFrame step={activeProof} onOpenImage={(src) => { setSelectedImage(src); trackEvent("homepage_proof_open", { proof_step: activeProof.id, asset: src }); }} /><div className="mt-6"><p className="text-sm font-semibold uppercase tracking-[0.2em] text-cyan-300/75">{activeProof.label}</p><h3 className="mt-3 text-2xl font-semibold text-white md:text-3xl">{activeProof.title}</h3><p className="mt-4 text-base leading-7 text-slate-400">{activeProof.body}</p></div></div></div><p className="mt-10 border-l-2 border-cyan-300/50 pl-5 text-lg font-medium leading-8 text-slate-300">The value is not a prettier dashboard. It is a more useful place to begin.</p></section>
