...
export default function AlternateHomePage() {
  ...
  usePageMeta({ title: "StrategicAI | See How Your Business Actually Works", description: "StrategicAI brings scattered business knowledge into a shared operating picture you can inspect, correct, and question before deciding what deserves action." });
    ...
    <div className="min-h-screen bg-slate-950 text-white selection:bg-cyan-400/20">
      <main>
        <section className="relative overflow-hidden border-b border-slate-800/70">
          ...
          <div className="relative mx-auto max-w-6xl px-6 pb-24 pt-28">
            <div className="max-w-4xl">
              ...
              <p className="mt-8 max-w-3xl text-lg leading-8 text-slate-300 md:text-2xl md:leading-10">Your people, systems, files, meetings, and memory each hold part of the answer. StrategicAI brings the available evidence into a shared operating picture that people can inspect, challenge, and correct.</p>
        ...
        <section className={sectionClass} aria-labelledby="operating-problem-title">
          <div className="mb-10 max-w-3xl"><p className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] text-cyan-300/75">You know these moments</p><h2 id="operating-problem-title" className="text-3xl font-semibold text-white md:text-5xl">Everyone has a piece.</h2><p className="mt-6 text-lg leading-8 text-slate-400">Your systems don’t contain everything your business knows. The business is moving. You’re still reconstructing the story. Different people see different parts. History lives in conversations and memory. The company collectively knows more than any one person can see.</p></div>
        ...
        <section className={`${sectionClass} pt-0`} aria-labelledby="reframe-title"><div className="grid items-stretch gap-8 lg:grid-cols-[1.05fr_0.95fr]"><div className={cardClass}><p className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] text-cyan-300/75">The Reframe</p><h2 id="reframe-title" className="text-3xl font-semibold text-white md:text-5xl">Your business already has an operating model. It just may not be visible.</h2><p className="mt-6 text-lg leading-8 text-slate-300">Operational Reality is the business itself: people, work, systems, decisions, dependencies, exceptions, history, and different perspectives. StrategicAI brings the available evidence about that reality into a shared operating picture you can challenge.</p><p className="mt-6 text-lg font-medium leading-8 text-cyan-100">AI makes it easier to change a business quickly. That makes it more important to understand what you are changing first.</p></div><div className="flex flex-col justify-center rounded-3xl border border-amber-200/25 bg-[linear-gradient(145deg,rgba(70,55,30,0.4),rgba(15,23,42,0.86))] p-8 md:p-10"><p className="text-sm font-semibold uppercase tracking-[0.24em] text-amber-200/80">Before action</p><p className="mt-6 text-3xl font-semibold leading-tight text-white md:text-4xl">Start with the picture. Then decide what deserves action—or what should stay unchanged.</p><div className="mt-8 flex flex-wrap items-center gap-3 text-sm text-slate-300">{["clarify ownership", "fix a handoff", "investigate", "automate when the picture supports it", "leave it alone"].map((item) => <span key={item} className="rounded-full border border-slate-700 bg-slate-950/30 px-3 py-2">{item}</span>)}</div></div></div></section>
        ...
        <section className={sectionClass} aria-labelledby="nemo-title"><div className="grid items-stretch gap-8 lg:grid-cols-[1.02fr_0.98fr]"><div className={cardClass}><p className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] text-cyan-300/75">Ask Nemo</p><h2 id="nemo-title" className="text-3xl font-semibold text-white md:text-5xl">What would you Ask Nemo first?</h2><p className="mt-6 text-lg leading-8 text-slate-300">Once the company picture is available, Ask Nemo gives you somewhere real to start.</p><p className="mt-6 text-lg leading-8 text-slate-300">How much better is AI when you don’t spend half the conversation teaching it what world it is operating inside? Ask Nemo.</p><p className="mt-6 text-lg leading-8 text-slate-300">Bring the thing bothering you and follow the trail.</p><p className="mt-6 text-base leading-7 text-slate-400">Nemo does not guarantee an answer. Ask Nemo helps you reason from the company context that is actually available.</p></div><div className="rounded-3xl border border-cyan-400/20 bg-[linear-gradient(145deg,rgba(9,42,49,0.7),rgba(15,23,42,0.9))] p-8"><p className="text-sm font-semibold uppercase tracking-[0.2em] text-cyan-300/75">What would you Ask Nemo?</p><div className="mt-5 space-y-3">{nemoQuestions.map((item) => { const active = selectedQuestion === item.id; return <button type="button" key={item.id} onClick={() => { setSelectedQuestion(item.id); trackEvent("homepage_nemo_question_select", { question_id: item.id }); }} aria-pressed={active} className={`w-full rounded-2xl border px-4 py-4 text-left text-sm transition ${active ? "border-cyan-300/35 bg-cyan-300/10 text-white" : "border-slate-700/70 bg-slate-950/30 text-slate-300 hover:border-slate-600"}`}>“{item.question}”</button>; })}</div><div className="mt-6 border-t border-slate-700/70 pt-6"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">What this context lets you follow</p><p className="mt-3 text-base leading-7 text-slate-200">{activeQuestion.response}</p></div></div></div></section>
        ...
        <section className={sectionClass} aria-labelledby="outcomes-title"><div className="mb-10 max-w-3xl"><p className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] text-cyan-300/75">The executive difference</p><h2 id="outcomes-title" className="text-3xl font-semibold text-white md:text-5xl">What gets better when the picture is shared?</h2><p className="mt-6 text-lg leading-8 text-slate-400">The sequence is designed to make these outcomes more possible—not to promise them automatically. The picture remains bounded by the evidence available and open to correction.</p></div><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{executiveOutcomes.map(([title, body]) => <article key={title} className={cardClass}><h3 className="text-xl font-semibold text-white">{title}</h3><p className="mt-4 text-base leading-7 text-slate-400">{body}</p></article>)}</div></section>
