...
const recognitionMoments = [
  ...
  ["The client asks for a fix before the problem is understood.", "Better implementation starts with better context."],
] as const;
...
const commitments = [
  ...
  "Claims only where the evidence supports them.",
] as const;

export default function Partners() {
  usePageMeta({
    title: "StrategicAI Partners | Better Context Before Implementation",
    description: "Bring better context into implementation by helping clients form a shared operating picture people can inspect and correct before consequential change.",
    path: "/partners",
  });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <main>
        <section className="relative overflow-hidden border-b border-slate-800/70"><div className="absolute inset-0"><img src="/images/brain-bg.jpg" alt="" className="h-full w-full object-cover opacity-[0.08]" /><div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(34,211,238,0.16),transparent_42%),linear-gradient(180deg,rgba(2,6,23,0.4),#020617_82%)]" /></div><div className="relative mx-auto max-w-6xl px-6 pb-24 pt-28"><div className="max-w-4xl"><p className="mb-6 text-sm font-semibold uppercase tracking-[0.28em] text-cyan-300/80">Partners</p><h1 className="text-5xl font-semibold leading-[1.05] text-white md:text-7xl">Bring better context into the work before implementation begins.</h1><p className="mt-8 max-w-3xl text-lg leading-8 text-slate-300 md:text-2xl md:leading-10">Your client already has pieces of the operating story spread across people, systems, decisions, history, and exceptions. StrategicAI helps turn those pieces into a shared operating picture that the people closest to the work can inspect and correct before consequential change begins.</p><div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center"><Link to="/founding100/offer" onClick={handleFounding100Click} className="inline-flex items-center justify-center rounded-full bg-cyan-400 px-7 py-4 text-base font-semibold text-slate-950 transition hover:bg-cyan-300">Experience StrategicAI through Founding 100 <ArrowRight size={17} className="ml-2" aria-hidden="true" /></Link><Link to="/how-it-works" className="inline-flex items-center justify-center rounded-full border border-slate-700 px-7 py-4 text-base font-semibold text-white transition hover:border-slate-500 hover:bg-slate-900">See how it works</Link></div></div></div></section>
        ...
        <section className={`${sectionClass} pt-0`} aria-labelledby="relationship-title"><div className="grid items-start gap-10 lg:grid-cols-[0.9fr_1.1fr] "><div><p className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] text-cyan-300/75">Better context before implementation</p><h2 id="relationship-title" className="text-3xl font-semibold text-white md:text-5xl">Work from a picture people can challenge.</h2><p className="mt-6 text-lg leading-8 text-slate-300">StrategicAI helps make the people, work, systems, dependencies, exceptions, history, and unknowns around the problem visible. The people closest to the work can correct the picture before your expertise moves the work downstream.</p><p className="mt-5 text-base leading-7 text-slate-400">That shared context can reduce the management drag of restarting discovery, reconciling assumptions, and asking the owner to become the historian for every specialist.</p></div><div className="rounded-3xl border border-cyan-400/20 bg-[linear-gradient(145deg,rgba(9,42,49,0.7),rgba(15,23,42,0.9))] p-8"><div className="flex flex-wrap items-center gap-3 text-sm text-slate-300">{["Available evidence", "Shared operating picture", "Human correction", "Better implementation"].map((item, index) => <span key={item} className="inline-flex items-center gap-3"><span className="rounded-full border border-slate-700 bg-slate-950/40 px-4 py-2">{item}</span>{index < 3 ? <ArrowRight size={15} className="text-cyan-200" aria-hidden="true" /> : null}</span>)}</div><p className="mt-8 text-base leading-7 text-slate-300">The owner, advisor, automation partner, systems specialist, or domain expert can see what the others are seeing before the work moves downstream.</p></div></div></section>
