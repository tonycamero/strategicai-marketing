...
const capabilities = [
  ...
  "Examine what a change may affect",
...
const architectureOutcomes = [
  ["Operational Reality", "Makes the context behind executive visibility available without claiming the picture is complete."],
  ["Operational Intelligence", "Turns the available picture into usable understanding for earlier visibility, alignment, and decision support."],
  ["Business Views", "Help leaders inspect the operating issue behind a question instead of rebuilding the whole story."],
  ["Nemo", "Lets frontier intelligence reason from available company context so answers can be examined against what is supported, disputed, and unknown."],
  ["Capabilities", "Let accountable people decide what deserves action without assuming automation is always the answer."],
] as const;
...
export default function Product() {
  usePageMeta({
    ...
    description: "See how StrategicAI makes organizational reality available as shared, correctable context for Operational Intelligence, Business Views, Nemo, and warranted action.",
    path: "/product",
  });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <main>
        ...
        <section className={`${sectionClass} pt-0`} aria-labelledby="grounding-title"><div className="grid items-stretch gap-8 lg:grid-cols-[0.9fr_1.1fr]"><div className="rounded-3xl border border-amber-200/25 bg-[linear-gradient(145deg,rgba(70,55,30,0.4),rgba(15,23,42,0.86))] p-8 md:p-10"><p className="text-sm font-semibold uppercase tracking-[0.24em] text-amber-200/80">The intelligence/context distinction</p><h2 id="grounding-title" className="mt-5 text-3xl font-semibold text-white md:text-5xl">Frontier models provide intelligence. StrategicAI provides organizational reality.</h2></div><div className={cardClass}><p className="text-lg leading-8 text-slate-300">Highly capable AI can still reason against incomplete, inconsistent, transient, or stale organizational context. StrategicAI makes an evidence-bounded representation of the business available to the intelligence layer so people can inspect and correct the context behind the reasoning.</p><p className="mt-6 text-base leading-7 text-slate-400">The jobs stay distinct: models reason from the available context; people inspect, dispute, and correct the picture. StrategicAI does not claim the picture is complete or automatically current.</p></div></div></section>

        <section className={`${sectionClass} pt-0`} aria-labelledby="intelligence-title"><div className="rounded-3xl border border-cyan-400/20 bg-[linear-gradient(145deg,rgba(9,42,49,0.7),rgba(15,23,42,0.9))] p-8 md:p-12"><div className="grid items-center gap-10 lg:grid-cols-[0.9fr_1.1fr]"><div><p className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] text-cyan-300/75">02 · Operational Intelligence</p><h2 id="intelligence-title" className="text-3xl font-semibold text-white md:text-5xl">Usable understanding formed from the picture.</h2><p className="mt-6 text-lg leading-8 text-slate-300">Operational Intelligence begins when the available picture can be inspected, corrected, and used for the questions and decisions actually in front of you. The business understanding beneath the intelligence layer can become more useful through evidence, correction, and use. It supports judgment; it does not become authority by itself.</p></div><div className="grid gap-4 sm:grid-cols-3">{intelligenceMoves.map(([title, body], index) => <div key={title} className="rounded-2xl border border-slate-700/80 bg-slate-950/35 p-5"><span className="text-xs font-semibold tracking-[0.18em] text-cyan-300/70">0{index + 1}</span><h3 className="mt-4 text-lg font-semibold text-white">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-400">{body}</p></div>)}</div></div><Link to="/#proof-thread" className="mt-10 inline-flex items-center text-sm font-semibold text-cyan-200 transition hover:text-cyan-100">See the illustrative example <ArrowRight size={16} className="ml-2" /></Link></div></section>
