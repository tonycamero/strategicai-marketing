...
export default function Product() {
  usePageMeta({
    title: "StrategicAI Product — Operational Intelligence for Your Business",
    description: "Turn what your people, systems, work, decisions, and exceptions already know into Operational Intelligence you can inspect, correct, and use.",
    path: "/product",
  });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <main>
        ...
        <section className={`${sectionClass} pt-0`} aria-labelledby="nemo-title"><div className="grid items-stretch gap-8 lg:grid-cols-[1.05fr_0.95fr]"><div className={cardClass}><p className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] text-cyan-300/75">04 · Ask Nemo</p><h2 id="nemo-title" className="text-3xl font-semibold text-white md:text-5xl">What would you Ask Nemo first?</h2><p className="mt-6 text-lg leading-8 text-slate-300">When you Ask Nemo, it works from the company context StrategicAI has made available. The point is not a better blank chat box. The point is being able to start with the business already in the room.</p><div className="mt-8 flex items-center gap-4 text-sm text-slate-300"><MessageCircle size={19} className="text-cyan-300" aria-hidden="true" /><span>Ask from the company—not from a generic prompt.</span></div></div><div className="rounded-3xl border border-slate-800 bg-slate-900/55 p-8"><p className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] text-cyan-300/75">05 · What happens next</p><h2 className="text-3xl font-semibold text-white">Then decide what deserves action.</h2><ul className="mt-7 space-y-4 text-base leading-7 text-slate-300">{capabilities.map((item) => <li key={item} className="flex gap-3"><CheckCircle2 size={18} className="mt-1 shrink-0 text-cyan-300" aria-hidden="true" />{item}</li>)}</ul><div className="mt-8 flex gap-3 text-sm text-slate-400"><GitBranch size={18} className="mt-0.5 shrink-0 text-amber-200" aria-hidden="true" /><span>Choose what follows once the picture makes the choice clearer.</span></div></div></div></section>

        <section className={sectionClass} aria-labelledby="product-next-title"><div className="rounded-3xl border border-slate-800 bg-slate-900/55 p-8 text-center md:p-12"><ShieldCheck size={28} className="mx-auto text-cyan-300" aria-hidden="true" /><h2 id="product-next-title" className="mt-6 text-3xl font-semibold text-white md:text-5xl">Understand the business. Then decide what comes next.</h2><p className="mx-auto mt-6 max-w-3xl text-lg leading-8 text-slate-300">See the proof, or enter the Founding 100 as the current path for building this kind of understanding around your own business.</p><div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row"><Link to="/#proof-thread" className="inline-flex items-center rounded-full border border-slate-700 px-6 py-3.5 text-sm font-semibold text-white transition hover:border-slate-500 hover:bg-slate-950">See the proof</Link><Link to="/founding100/offer" onClick={handleFounding100Click} className="inline-flex items-center rounded-full bg-cyan-400 px-6 py-3.5 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300">Explore Founding 100 <ArrowRight size={16} className="ml-2" aria-hidden="true" /></Link></div></div></section>
      </main>
